 ...
 */
async function tryGeneratorScaffold(domain: DomainInfo, config: GeneratorConfig) {
  ...
  try {
    ...
    if (!fs.existsSync(generatorPath)) {
      console.warn(
        `[Contract Scaffolder] Generator not found at: ${generatorPath}, using manual scaffolding`
      );
      return null;
    }
    ...
    console.warn(
      `[Contract Scaffolder] Generator failed for ${domain.name}:`,
 ...
 */
async function manualScaffold(domain: DomainInfo, config: GeneratorConfig) {
  const outputPath = getContractOutputPath(config.output, domain.name);
